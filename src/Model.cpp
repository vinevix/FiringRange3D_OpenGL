...
namespace objloader
{
	...
	bool Model::load(int targetId, const std::string& data_path, const std::string& mesh_filename, const std::string& material_filename)
	{
		dataPath_ = data_path;
		...
		glEnable(GL_TEXTURE_2D);
		listIdPicking = glGenLists(1);
		glNewList(listIdPicking, GL_COMPILE);

		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

		buildPicking(targetId);
	...
	{
		if (line.find("#") == 0) // commento
		{
			return;
		}
		else if (line.find("newmtl ") == 0) //  definisce un nuovo materiale
		{
		...
		else if (line.find("Ka ") == 0) // colore ambientale
		{
			std::vector<std::string> tokens;
			split(line, ' ', tokens);
	...
	void Model::buildPicking(float targetId)
	{
		glEnable(GL_NORMALIZE);
		glDisable(GL_TEXTURE_2D);
		...
		for (int i = 0; i < object_.faces.size(); ++i) // Each face
		{
			...
			glBegin(GL_POLYGON);
			glColor4f(0.4 + (float)(targetId / 10), 0.7 + (float)(2 * targetId / 10), 0.94, 1);
			for (int j = 0; j < object_.faces.at(i).items.size(); ++j) // Each vertex
			{
				OBJFaceItem& item = object_.faces.at(i).items.at(j);
	...
	void Model::render(bool picking, bool withBoundingBox) const
	{
		if (listId_ != 0 && picking == false)
			glCallList(listId_);
		else if (listIdPicking != 0 && picking == true)
			glCallList(listIdPicking);
