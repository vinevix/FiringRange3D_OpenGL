...
namespace objloader
{
	...
	public:
		 ...
		 */
		bool load(int targetId, const std::string& data_path, const std::string& mesh_filename, const std::string& material_filename = "");

		/**
		 * \brief Render the model
		 * \param withBoundingBox true if the bounding box should be rendered as well
		 */
		void render(bool picking, bool withBoundingBox = false) const;
	...
	protected:
		 ...
		 */
		void build();
		void buildPicking(float targetId);
